...
Install:
```bash
$ cd ~/catkin_ws && rosdep install --from-paths src --ignore-src -r
...
This version is ready to read Range Profile Data with zero dopler. 

To run with our hardware (AWR1843BOOST) run:
...
/ti_mmwave/range_profile         : sensor_msgs/ChannelFloat32

A csv file will automaticly be created in the source of this package. 

**To publish to nodered run:**
```bash
$ rosrun ti_mmwave_rospkg radar_node_red_node _radar_rate:=20 _nodered_topic:=/radar_topic
```




[Origianl Repository HERE: ](https://github.com/radar-lab/ti_mmwave_rospkg)
